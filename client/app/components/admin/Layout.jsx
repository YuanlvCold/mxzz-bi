...
export default function Layout({ activeTab, children }) {
  return (
    <div className="container admin-page-layout">
      <PageHeader title={__("Admin")} />

      <div className="bg-white tiled">
        <Tabs className="admin-page-layout-tabs" defaultActiveKey={activeTab} animated={false} tabBarGutter={0}>
          <Tabs.TabPane key="system_status" tab={<a href="admin/status">{__("System Status")}</a>}>
            {activeTab === "system_status" ? children : null}
          </Tabs.TabPane>
          <Tabs.TabPane key="jobs" tab={<a href="admin/queries/jobs">{__("RQ Status")}</a>}>
            {activeTab === "jobs" ? children : null}
          </Tabs.TabPane>
          <Tabs.TabPane key="outdated_queries" tab={<a href="admin/queries/outdated">{__("Outdated Queries")}</a>}>
