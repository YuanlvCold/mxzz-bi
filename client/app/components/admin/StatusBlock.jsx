...
export function General({ info }) {
  info = toPairs(info);
  return (
    <Card title={__("General")} size="small">
      {info.length === 0 && <div className="text-muted text-center">{__("No data")}</div>}
      {info.length > 0 && (
        <List
          size="small"
          itemLayout="vertical"
...
export function DatabaseMetrics({ info }) {
  return (
    <Card title={__("Redash Database")} size="small">
      {info.length === 0 && <div className="text-muted text-center">{__("No data")}</div>}
      {info.length > 0 && (
        <List
          size="small"
...
export function Queues({ info }) {
  info = toPairs(info);
  return (
    <Card title={__("Queues")} size="small">
      {info.length === 0 && <div className="text-muted text-center">{__("No data")}</div>}
      {info.length > 0 && (
        <List
          size="small"
  ...
  const items = info
    ? [
        ...
        <List.Item extra={<span className="badge">{info.outdatedQueriesCount}</span>}>
          {__("Outdated Queries Count")}
        </List.Item>,
      ]
    : [];

  return (
    <Card title={__("Manager")} size="small">
      {!info && <div className="text-muted text-center">{__("No data")}</div>}
