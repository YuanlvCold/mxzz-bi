  ...
  render() {
    ...
    return (
      <Modal
        ...
        title={__("Embed Query")}
        footer={<Button onClick={dialog.dismiss}>{__("Close")}</Button>}>
        {query.is_safe ? (
          <React.Fragment>
            <h5 className="m-t-0">{__("Public URL")}</h5>
            <div className="m-b-30">
              <CodeBlock data-test="EmbedIframe" copyable>
                {this.embedUrl}
              </CodeBlock>
            </div>
            <h5 className="m-t-0">{__("IFrame Embed")}</h5>
            <div>
              <CodeBlock copyable>
          ...
          <Alert
            message={__("Currently it is not possible to embed queries that contain text parameters.")}
            type="error"
