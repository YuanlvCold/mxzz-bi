...
function ApiKeyDialog({ dialog, ...props }) {
  ...
  const regenerateQueryApiKey = useCallback(() => {
    ...
    axios
      ...
      .catch(() => {
        setUpdatingApiKey(false);
        notification.error(__("Failed to update API key"));
      });
  }, [query]);
  ...
  return (
    <Modal {...dialog.props} width={600} footer={<Button onClick={() => dialog.close(query)}>{__("Close")}</Button>}>
      <div className="query-api-key-dialog-wrapper">
        ...
        <h5>{__("Example API Calls")}:</h5>
        <div className="m-b-10">
          <label>{__("Results in CSV format")}:</label>
          <CodeBlock copyable>{csvUrl}</CodeBlock>
        </div>
        <div>
          <label>{__("Results in JSON format")}:</label>
