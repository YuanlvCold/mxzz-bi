...
function saveVisualization(visualization) {
  ...
  return Visualization.save(visualization)
    .then(result => {
      notification.success(__("Visualization saved"));
      return result;
    })
    .catch(error => {
      notification.error(__("Visualization could not be saved"));
...
function confirmDialogClose(isDirty) {
  return new Promise((resolve, reject) => {
    if (isDirty) {
      Modal.confirm({
        title: __("Visualization Editor"),
        content: __("Are you sure you want to close the editor without saving?"),
        okText: __("Yes"),
        cancelText: __("No"),
        onOk: () => resolve(),
        onCancel: () => reject(),
      ...
      <div className="edit-visualization-dialog">
        <div className="visualization-settings">
          <div className="m-b-15">
            <label htmlFor="visualization-type">{__("Visualization Type")}</label>
            <Select
          ...
          <div className="m-b-15">
            <label htmlFor="visualization-name">{__("Visualization Name")}</label>
            <Input
