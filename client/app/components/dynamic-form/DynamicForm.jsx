...
const fieldRules = ({ type, required, minLength }) => {
  ...
  return [
    requiredRule && { required, message: __("This field is required.") },
    minLengthRule && { min: minLength, message: __("This field is too short.") },
    emailTypeRule && { type: "email", message: __("This field must be a valid email.") },
  ].filter(rule => rule);
};
    ...
    const upload = (
      <Upload {...props} beforeUpload={() => false}>
        <Button disabled={disabled}>
          <Icon type="upload" /> {__("Click to upload")}
        </Button>
      </Upload>
    );
