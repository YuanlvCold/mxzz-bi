...
export default class UserEdit extends React.Component {
  ...
  regenerateApiKey = () => {
    ...
    Modal.confirm({
      title: __("Regenerate API Key"),
      content: __("Are you sure you want to regenerate?"),
      okText: __("Regenerate"),
      onOk: doRegenerate,
      maskClosable: true,
  ...
  saveUser = (values, successCallback, errorCallback) => {
    ...
    User.save(data)
      .then(user => {
        successCallback(__("Saved."));
        this.setState({ user: User.convertUserInfo(user) });
      })
      .catch(error => {
        errorCallback(get(error, "response.data.message", __("Failed saving.")));
      });
  };
    ...
    const formFields = [
      ...
      !user.isDisabled && currentUser.id !== user.id
        ? {
            ...
            placeholder: loadingGroups ? __("Loading...") : "",
          }
        : {
            name: "group_ids",
            title: __("Groups"),
  ...
  renderApiKey() {
    ...
    return (
      <Form layout="vertical">
        <hr />
        <Form.Item label={__("API Key")} className="m-b-10">
    ...
    return (
      <Alert
        ...
        description={
          <Fragment>
            <p>
              {__("The mail server is not configured, please send the following link to")} <b>{user.name}</b>:
            </p>
