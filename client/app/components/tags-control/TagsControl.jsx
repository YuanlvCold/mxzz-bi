...
function modelTagsControl({ archivedTooltip }) {
  ...
  function ModelTagsControl({ isDraft, isArchived, ...props }) {
    return (
      <TagsControl {...props}>
        {!isArchived && isDraft && <span className="label label-tag-unpublished">{__("Unpublished")}</span>}
        {isArchived && (
          <Tooltip placement="right" title={archivedTooltip}>
            <span className="label label-tag-archived">{__("Archived")}</span>
          </Tooltip>
        )}
      </TagsControl>
    );
...
export const QueryTagsControl = modelTagsControl({
  archivedTooltip: __("This query is archived and can't be used in dashboards, or appear in search results."),
});

export const DashboardTagsControl = modelTagsControl({
  archivedTooltip: __("This dashboard is archived and won't be listed in dashboards nor search results."),
});
