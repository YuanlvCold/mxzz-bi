...
class CreateSourceDialog extends React.Component {
    ...
    return (
      <div>
        ...
        {selectedType.type === "databricks" && (
          <small>
            {__("By using the Databricks Data Source you agree to the Databricks JDBC/ODBC")}{" "}
            <a href="https://databricks.com/spark/odbc-driver-download" target="_blank" rel="noopener noreferrer">
            {__("Driver Download Terms and Conditions")}
            </a>
            .
          </small>
        )}
  ...
  render() {
    ...
    return (
      <Modal
        {...dialog.props}
        title={`${__("Create a New")} ${sourceType}`}
        footer={
          currentStep === StepEnum.SELECT_TYPE
            ? [
        ...
        <div data-test="CreateSourceDialog">
          <Steps className="hidden-xs m-b-10" size="small" current={currentStep} progressDot>
            {currentStep === StepEnum.CONFIGURE_IT ? (
              <Step title={<a>{__("Type Selection")}</a>} className="clickable" onClick={this.resetType} />
            ) : (
              <Step title={__("Type Selection")} />
            )}
            <Step title={__("Configuration")} />
            <Step title={__("Done")} />
          </Steps>
