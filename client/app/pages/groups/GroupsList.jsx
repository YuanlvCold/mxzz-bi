...
class GroupsList extends React.Component {
  ...
  listColumns = [
    Columns.custom(
      (text, group) => (
        <div>
          ...
          {group.type === "builtin" && <span className="label label-default m-l-10">{__("built-in")}</span>}
        </div>
      ),
      {
        field: "name",
    ...
    Columns.custom(
      (text, group) => (
        <Button.Group>
          <Button href={`groups/${group.id}`}>{__("Members")}</Button>
          {currentUser.isAdmin && <Button href={`groups/${group.id}/data_sources`}>{__("Data Sources")}</Button>}
        </Button.Group>
      ),
      {
        width: "1%",
      ...
      (text, group) => {
        ...
        return (
          <DeleteGroupButton
            ...
            disabled={!canRemove}
            group={group}
            title={canRemove ? null : __("Cannot delete built-in group")}
