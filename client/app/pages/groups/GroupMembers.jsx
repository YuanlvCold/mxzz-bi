...
class GroupMembers extends React.Component {
  ...
  removeGroupMember = (event, user) =>
    Group.removeMember({ id: this.groupId, userId: user.id })
      ...
      .catch(() => {
        notification.error(__("Failed to remove member from group."));
      });

  addMembers = () => {
    const alreadyAddedUsers = map(this.props.controller.allItems, u => u.id);
    SelectItemsDialog.showModal({
      dialogTitle: __("Add Members"),
      inputPlaceholder: __("Search users..."),
      selectedItemsTitle: __("New Members"),
          ...
          <Layout.Content>
            ...
            {controller.isLoaded && controller.isEmpty && (
              <div className="text-center">
                <p>{__("There are no members in this group yet.")}</p>
                {currentUser.isAdmin && (
