...
class OrganizationSettings extends React.Component {
  ...
  renderGeneralSettings() {
    ...
    return (
      <React.Fragment>
        <h3 className="m-t-0">{__("General")}</h3>
        <hr />
        <Form.Item label={__("Date Format")}>
          <Select
        ...
        <Form.Item label={__("Time Format")}>
          <Select
        ...
        <Form.Item label={__("Chart Visualization")}>
          <Checkbox
            ...
            {__("Hide Plotly mode bar")}
          </Checkbox>
        </Form.Item>
        <Form.Item label={__("Feature Flags")}>
          <Checkbox
            ...
            {__("Enable experimental multiple owners support")}
          </Checkbox>
        </Form.Item>
        <Form.Item>
          <Checkbox
            ...
            {__("Email query owners when scheduled queries fail")}
          </Checkbox>
        </Form.Item>
        <Form.Item>
          <Checkbox
            ...
            {__("Enable multi-byte (Chinese, Japanese, and Korean) search for query names and descriptions (slower)")}
          </Checkbox>
        </Form.Item>
        <DynamicComponent name="BeaconConsentSetting">
          <Form.Item
            label={
              <>
                {__("Anonymous Usage Data Sharing")} <HelpTrigger type="USAGE_DATA_SHARING" />
              </>
            }>
            <Checkbox
              name="beacon_consent"
              ...
              {__("Help Redash improve by automatically sending anonymous usage data")}
            </Checkbox>
          </Form.Item>
        </DynamicComponent>
      </React.Fragment>
  ...
  renderAuthSettings() {
    ...
    return (
      <React.Fragment>
        <h3 className="m-t-0">
          {__("Authentication")} <HelpTrigger type="AUTHENTICATION_OPTIONS" />
        </h3>
        <hr />
        {!settings.auth_password_login_enabled && (
          <Alert
            message={__(`Password based login is currently disabled and users will
            ...
            <Tooltip
              title={
                this.disablePasswordLoginToggle()
                  ? __("Password login can be disabled only if another login method is enabled.")
                  : null
              }
              placement="right">
              {__("Password Login Enabled")}
            </Tooltip>
          </Checkbox>
        </Form.Item>
