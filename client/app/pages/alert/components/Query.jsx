  ...
  const queryHint =
    query && query.schedule ? (
      ...
      <small>
        ...
        .{" "}
        <Tooltip title={__("A query schedule is not necessary but is highly recommended for alerts. An Alert without a query schedule will only send notifications if a user in your organization manually executes this query.")}>
          <a>
            {__("Why it&apos;s recommended")} <Icon type="question-circle" theme="twoTone" />
          </a>
        </Tooltip>
      </small>
    );
  ...
  return (
    <>
      {editMode ? (
        ...
        <Tooltip title={__("Open query in a new tab.")}>
      ...
      {query && !queryResult && (
        <div className="m-t-30">
          <Icon type="loading" className="m-r-5" /> {__("Loading query data")}
        </div>
      )}
    </>
  );
