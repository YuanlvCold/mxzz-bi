...
class Alert extends React.Component {
        ...
        .then(alert => {
          if (this._isMounted) {
            ...
            if (!canEdit) {
              ...
              notification.warn(
                __("You cannot edit this alert"),
                __("You do not have sufficient permissions to edit this alert, and have been redirected to the view-only page."),
                { duration: 0 }
              );
            }
  ...
  save = () => {
    ...
    return AlertService.save(alert)
      .then(alert => {
        notification.success(__("Saved."));
        navigateTo(`alerts/${alert.id}`, true);
        this.setState({ alert, mode: MODES.VIEW });
      })
      .catch(() => {
        notification.error(__("Failed saving alert."));
      });
  };
  ...
  delete = () => {
    ...
    return AlertService.delete(alert)
      .then(() => {
        notification.success(__("Alert deleted successfully."));
        navigateTo("alerts");
      })
      .catch(() => {
        notification.error(__("Failed deleting alert."));
      });
  };
  ...
  mute = () => {
    ...
    return AlertService.mute(alert)
      .then(() => {
        ...
        notification.warn(__("Notifications have been muted."));
      })
      .catch(() => {
        notification.error(__("Failed muting notifications."));
      });
  };
  ...
  unmute = () => {
    ...
    return AlertService.unmute(alert)
      .then(() => {
        ...
        notification.success(__("Notifications have been restored."));
      })
      .catch(() => {
        notification.error(__("Failed restoring notifications."));
      });
  };
