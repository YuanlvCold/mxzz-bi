...
class OutdatedQueries extends React.Component {
  ...
  listColumns = [
    ...
    Columns.avatar({ field: "user", className: "p-l-0 p-r-0" }, name => `__("Created by") ${name}`),
    Columns.dateTime.sortable({ title: __("Created At"), field: "created_at" }),
    Columns.duration.sortable({ title: __("Runtime"), field: "runtime" }),
    Columns.dateTime.sortable({ title: __("Last Executed At"), field: "retrieved_at", orderByField: "executed_at" }),
        ...
        <Grid.Row className="m-15">
          <Grid.Col span={16}>
            ...
            {controller.params.lastUpdatedAt && (
              <div className="m-t-5">
                {__("Last updated")}: <TimeAgo date={controller.params.lastUpdatedAt * 1000} />
              </div>
            )}
          </Grid.Col>
          <Grid.Col span={8}>
