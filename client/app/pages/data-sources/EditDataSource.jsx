...
class EditDataSource extends React.Component {
  ...
  saveDataSource = (values, successCallback, errorCallback) => {
    ...
    DataSource.save(dataSource)
      ...
      .catch(error => {
        const message = get(error, "response.data.message", __("Failed saving."));
  ...
  deleteDataSource = callback => {
    ...
    const doDelete = () => {
      DataSource.delete(dataSource)
        .then(() => {
          notification.success(__("Data source deleted successfully."));
    ...
    Modal.confirm({
      ...
      content: __("Are you sure you want to delete this data source?"),
      okText: __("Delete"),
      okType: "danger",
      onOk: doDelete,
      onCancel: callback,
  ...
  testConnection = callback => {
    ...
    DataSource.test({ id: dataSource.id })
      .then(httpResponse => {
        if (httpResponse.ok) {
          notification.success(__("Success"));
        } else {
          notification.error(__("Connection Test Failed:"), httpResponse.message, { duration: 10 });
        }
        callback();
      })
      .catch(() => {
        notification.error(
          ...
          __("Unknown error occurred while performing connection test. Please try again later."),
          { duration: 10 }
        );
        callback();
      });
  ...
  renderForm() {
    ...
    const formProps = {
      ...
      type,
      actions: [
        { name: __("Delete"), type: "danger", callback: this.deleteDataSource },
        { name: __("Test Connection"), pullRight: true, callback: this.testConnection, disableWhenDirty: true },
      ],
