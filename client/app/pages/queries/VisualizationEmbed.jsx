...
function VisualizationEmbedHeader({ queryName, queryDescription, visualization }) {
  return (
    <div className="embed-heading p-b-10 p-r-15 p-l-15">
      <h3>
        <img src={logoUrl} alt={__("Redash Logo")} style={{ height: "24px", verticalAlign: "text-bottom" }} />
    ...
    <Menu>
      <Menu.Item>
        <QueryResultsLink
          ...
          embed>
          <Icon type="file" /> {__("Download as CSV File")}
        </QueryResultsLink>
      </Menu.Item>
      <Menu.Item>
        <QueryResultsLink
          ...
          disabled={!queryResults || !queryResults.getData || !queryResults.getData()}
          embed>
          <Icon type="file" /> {__("Download as TSV File")}
        </QueryResultsLink>
      </Menu.Item>
      <Menu.Item>
        <QueryResultsLink
          ...
          disabled={!queryResults || !queryResults.getData || !queryResults.getData()}
          embed>
          <Icon type="file-excel" /> {__("Download as Excel File")}
        </QueryResultsLink>
      </Menu.Item>
    </Menu>
  );
...
function VisualizationEmbed({ queryId, visualizationId, apiKey, onError }) {
  ...
  if (!visualization) {
    ...
    setTimeout(() => {
      onError(new Error(__("Visualization does not exist")));
    }, 10);
    return null;
  }
