  ...
  return (
    <div className="query-execution-metadata">
      ...
      <span className="m-l-5 m-r-10">
        <span>
          <strong>{queryResultData.rows.length}</strong> {pluralize(__("row"), queryResultData.rows.length)}
        </span>
        <span className="m-l-5">
          {!isQueryExecuting && (
            <React.Fragment>
              <strong>{durationHumanize(queryResultData.runtime)}</strong>
              <span className="hidden-xs"> {__("runtime")}</span>
            </React.Fragment>
          )}
          {isQueryExecuting && <span>{__("Running")}&hellip;</span>}
        </span>
      ...
      <div>
        <span className="m-r-10">
          <span className="hidden-xs">{__("Refreshed")} </span>
          <strong>
