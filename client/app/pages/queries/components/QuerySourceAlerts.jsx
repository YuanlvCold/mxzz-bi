  ...
  if (queryFlags.isNew && !queryFlags.canCreate) {
    message = (
      <React.Fragment>
        <Typography.Title level={4}>
          {__("You don't have permission to create new queries on any of the data sources available to you")}.
        </Typography.Title>
        <p>
          <Typography.Text type="secondary">
            {__("You can either")} <a href="queries">{__("browse existing queries")}</a>, {__("or ask for additional permissions from your Redash admin")}.
          </Typography.Text>
        </p>
      </React.Fragment>
    );
    ...
    if (currentUser.isAdmin) {
      message = (
        <React.Fragment>
          <Typography.Title level={4}>
            {__("Looks like no data sources were created yet or none of them available to the group(s) you're member of")}.
          </Typography.Title>
          <p>
            <Typography.Text type="secondary">{__("Please create one first, and then start querying")}.</Typography.Text>
          </p>
      ...
      message = (
        <React.Fragment>
          ...
          <p>
            <Typography.Text type="secondary">{__("Please ask your Redash admin to create one first")}.</Typography.Text>
          </p>
        </React.Fragment>
      );
    }
